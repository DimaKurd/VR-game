...
namespace SDKTemplate
{
    ...
    public sealed partial class Scenario2_Client : Page
    {
        ...
        private async void Characteristic_ValueChanged(GattCharacteristic sender, GattValueChangedEventArgs args)
        {
            ...
            try
            {
                newValue = newValue.Substring(newValue.IndexOf(':') + 2);
                var responseString = await client.GetStringAsync($"http://127.0.0.1:8000/hr/update?bpm={newValue}");
            }
            catch (Exception e)
            {
        ...
        private string FormatValueByPresentation(IBuffer buffer, GattPresentationFormat format)
        {
            ...
            else if (data != null)
            {
                ...
                if (selectedCharacteristic.Uuid.Equals(GattCharacteristicUuids.HeartRateMeasurement))
                {
                    try
                    {
                        var bpm = ParseHeartRateValue(data).ToString();                  
